...
const PRICES = {
  ...
  carpet: { pricePerSqm: 13000 },     // plus de livraison/transport
  curtain: { dubai: 6000, quality2: 4500, quality3: 4000 }, // plus de livraison/transport
};
...
function distributeSegments(L, k) {
  if (k <= 0) return [];
  // priorité: segments égaux si possible
  const eq = L / k;
  if (eq >= MIN - 1e-9 && eq <= MAX + 1e-9) {
    const segs = Array(k).fill(eq);
    // correction minime de somme
    const sum = segs.reduce((a, b) => a + b, 0);
    segs[0] += L - sum;
    return segs;
  }
  // sinon, répartition quasi-égale dans [MIN,MAX]
  const segs = Array(k).fill(MIN);
  let rem = L - k * MIN;
...
function SalonDiagram({ geom, smallTableCount, bigTableCount }) {
  const { widthPx, heightPx, scale, rects, coins, tables } = useMemo(() => {
    ...
    if (geom.forme === "L") {
      ...
      bigCenters.push({ x: cx, y: cy });
      bigCenters.push({ x: cx + 0.9, y: cy });
      bigCenters.push({ x: cx - 0.9, y: cy });
      bigCenters.push({ x: cx, y: cy + 0.9 });
    } else {
      // petites : dans les coins intérieurs & bouts bas des 2 jambes
      smallCenters.push({ x: cL + 0.7, y: cL + 0.7 });                 // coin intérieur gauche
      smallCenters.push({ x: cL + effC - 0.7, y: cR + 0.7 });           // coin intérieur droit
      smallCenters.push({ x: cL + 0.7, y: cL + Math.max(0.8, effG - 0.7) }); // bout bas gauche
      smallCenters.push({ x: cL + effC - 0.7, y: cR + Math.max(0.8, effD - 0.7) }); // bout bas droit
      // grandes : milieu de la zone ouverte du U (grille)
      const cx = cL + effC / 2;
    ...
    // Répéter les positions si le nombre dépasse les emplacements "naturels"
    const placeMany = (centers, need) => {
      const out = [];
      let i = 0, ring = 0;
      while (out.length < need) {
        const base = centers[i % centers.length];
        const jitter = 0.25 * ring; // léger décalage pour empiler proprement
...
export default function QuotePage() {
  ...
  const buildWhatsappText = () => {
    ...
    const sides = g.perSide
      .map(s => `\n- ${s.id} utile ${s.effective.toFixed(2)} m | ${s.segments.length} matelas: ${s.segments.map(v=>v.toFixed(2)).join(" , ")} m`)
      .join("");
    const total = `\n\nTotal: ${price(calculatedPrice.total)} (Tous frais inclus)`;
    return `${header}${client}${shape}${coins}${tables}${sides}${total}\n\nMerci.`;
  };
                ...
                <CardContent className="space-y-6">
                  {salonData.shape === "L" ? (
                    <div className="grid md:grid-cols-3 gap-6">
                      ...
                      <div className="space-y-2"><Label>Côté A (m)</Label>
                        <Input type="number" step="0.1" min="0" value={salonData.sideA_m} onChange={(e)=>setSalonData({...salonData,sideA_m:Number(e.target.value)})}/>
                      </div>
                      <div className="space-y-2"><Label>Côté B (m)</Label>
                        <Input type="number" step="0.1" min="0" value={salonData.sideB_m} onChange={(e)=>setSalonData({...salonData,sideB_m:Number(e.target.value)})}/>
                      </div>
                    </div>
                  ) : (
                    <div className="grid md:grid-cols-4 gap-6">
                      ...
                      <div className="space-y-2"><Label>Gauche (m)</Label>
                        <Input type="number" step="0.1" min="0" value={salonData.sideA_m} onChange={(e)=>setSalonData({...salonData,sideA_m:Number(e.target.value)})}/>
                      </div>
                      <div className="space-y-2"><Label>Central (m)</Label>
                        <Input type="number" step="0.1" min="0" value={salonData.sideB_m} onChange={(e)=>setSalonData({...salonData,sideB_m:Number(e.target.value)})}/>
                      </div>
                      <div className="spacey-2"><Label>Droit (m)</Label>
                        <Input type="number" step="0.1" min="0" value={salonData.sideC_m} onChange={(e)=>setSalonData({...salonData,sideC_m:Number(e.target.value)})}/>
                      </div>
                    </div>
                  )}
